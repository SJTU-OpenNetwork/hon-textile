...
type StreamManager struct {
	blockFetcher func(streamId string, startIndex int) ([]cid.Cid, error)
	streamFetcher func(streamId string) (*Stream, error)
	activeStreams map[string] *Stream			// Cache active streams. (Maybe it is redundant.)
...
type streamWorker struct {
	stream *Stream // Contains core information such as substream number
}
...
func (sm *StreamManager) ResponseRequest(req *StreamRequest) error {
	_, ok := sm.activeStreams[req.StreamID]

	if !ok {
		stream, err := sm.streamFetcher(req.StreamID)
		if err != nil {
			return err
		}
		sm.activeStreams[req.StreamID] = stream
	}
...
func (sm *StreamManager) startNewWorker(req *StreamRequest) error {
