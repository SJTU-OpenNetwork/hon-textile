...
- External invites are encrypted with a single-use key and are useful for onboarding new users.`).Alias("invites")
	...
	// E.g. https://github.com/SJTU-OpenNetwork/hon-textile/blob/72a910879b5b8135d3cf65c5348beeb5aa4226a0/core/threads_service.go#L395
	...
	// https://github.com/SJTU-OpenNetwork/hon-textile/issues/823
...
The response contains a base58 encoded version of the random bytes token.`).Alias("create").Alias("generate").Alias("init")
	...
	streamListCmd := streamCmd.Command("list", "List all streams.")
	cmds[streamListCmd.FullCommand()] = func () error {
		return ListStream()
	}
	...
	thread2CreateCmd := thread2Cmd.Command("create", "Create a new thread.")
	cmds[thread2CreateCmd.FullCommand()] = Thread2Create

	thread2AddString := thread2Cmd.Command("addString", "Add a string record to thread.")
	thread2AddStringThreadId := thread2AddString.Arg("thread", "thread Id").Required().String()
	thread2AddStringText := thread2AddString.Arg("text", "").Required().String()
	cmds[thread2AddString.FullCommand()] = func() error {
